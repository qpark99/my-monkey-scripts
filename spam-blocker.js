...
(function() {
    ...
    const blockedBoards = ['stock'];

    function blockBoards() {
        if (!blockedBoards.length) return;
        const sel = blockedBoards.flatMap(b => [
            `.xe-widget-wrapper:has(a[href="${b}"])`,   // 메인 위젯 (제목 링크가 상대경로)
            `.xe-widget-wrapper:has(a[href="/${b}"])`,
    ...
    const keywords = [
        ...
        '빅히트', '뺑소니', '쿠팡', '파업', '삼전', '스벅', '스타벅스', '정용진', '신세계', '일베',
        'snl', 'SNL', '영포티', '짱깨', '우크라', '자살', '슈카', '동남아', '의새', '다케시마',
        '독도', '충주', '빽보이', '빽다방', '그알', '알고싶다', '앱스타인', '엡스타인', '오킹', '논란',
        '우왁굳', 'ㅇㅇㄱ', '굴단', '페미', '왁타', '🦪', '차은우', '탈세', '담합', '연봉',
        '실수령', '퐁퐁', '월급', '이스라엘', 'ㅇㅎ', 'ㅎㅂ', '선관위', '부정선거', '투표용지', '개표',
        '투표소', '전한길', '모스탄', '윤어게인', '선거무효', '검열', '허위조작정보', '탱크데이', '멸공', '환율',
        '코스피', '선거', '준동', '안협소', '시위', 'MBC', '왜곡', '투표율', '월드컵', '손흥민',
        '리센느', '트릭컬', '여경', '미실현', '주식', '폭락', '홍명보', '홍감독', '클린스만', '벤투',
        '황선홍', '신태용', '정몽규', '이임생', '이강인', '김민재', '황희찬', '황인범', '조규성', '오현규',
        '박지성', '이영표', '안정환', '감스트', '김병현', '김영광', '축협', '국축', '해축', 'A매치',
        '평가전', '예선', '본선', 'K리그', '피파', 'FIFA', '흥민', '몽규', '배재고', '김호중',
        '추태', '피카소', '졌잘싸', '파라과이', '16강', '총기', '난임병원', '냉부', '시크릿 책', '원피스',
        '아르헨티나', '레버리지', '홈플러스', 'MCU', '스파이더맨', '구마모토', '쿠마모토', '이온몰', '인권법', '하이닉스',
        '하닉', '혐한', '귀화', '일뽕', '나노하', '돌려차기', '활협전', '한요일', 'OCN', '호프',
        '음주운전', '국장', '미장', '성접대', '오디세이', '오딧세이', '그래미', '폭염', '열돔',
        ]; // 원하는 키워드로 수정
